...
const StatsGrid: React.FC = () => {
    return (
        <section className="bg-[#050505] text-white py-32 px-6 relative overflow-hidden border-t border-white/5">
            <div className="max-w-7xl mx-auto">
                <div className="mb-20 text-center">
                    <h2 className="text-3xl md:text-5xl font-heading font-bold mb-6 bg-clip-text text-transparent bg-gradient-to-b from-white to-white/40 tracking-tight">
                        Used by World's largest contractors.
                    </h2>
                    <p className="text-gray-500">Proven results across megaprojects globally.</p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 auto-rows-[minmax(180px,auto)]">
                    {stats.map((stat, index) => (
                        <motion.div
                            ...
                            className={`
                        relative group overflow-hidden rounded-2xl border border-white/5 bg-white/[0.02] hover:bg-white/[0.04] hover:border-white/10 transition-all duration-500 hover:scale-[1.02] p-8 flex flex-col justify-end
                        ${stat.size === 'large' ? 'md:col-span-2 md:row-span-2' : ''}
                        ${stat.size === 'wide' ? 'md:col-span-2' : ''}
                        ${stat.size === 'medium' ? 'md:col-span-1 md:row-span-1' : ''}
                        ${stat.size === 'small' ? 'md:col-span-1' : ''}
                    `}
                        >
                            <div className="absolute inset-0 bg-gradient-to-br from-white/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>

                            <h3 className="text-4xl md:text-5xl font-heading font-bold mb-4 bg-clip-text text-transparent bg-gradient-to-br from-white to-gray-500 tracking-tight">
                                {stat.value}
                            </h3>

...
const Home: React.FC = () => {
    return (
        <div className="relative h-screen bg-white text-black selection:bg-black/20 overflow-hidden">
            ...
            {/* Black spark glow in the center */}
            <div className="absolute inset-0 flex items-center justify-center z-10">
                ...
                >
                    ...
                    {/* Mid glow */}
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-24 h-24 rounded-full bg-black/20 blur-2xl" />
                    {/* Inner glow */}
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-black/40 blur-xl" />
                    {/* Core spark */}
                    <motion.div
                        animate={{
                            boxShadow: [
                                '0 0 20px 4px rgba(0,0,0,0.3), 0 0 60px 10px rgba(0,0,0,0.15)',
                                '0 0 30px 8px rgba(0,0,0,0.4), 0 0 80px 20px rgba(0,0,0,0.2)',
                                '0 0 20px 4px rgba(0,0,0,0.3), 0 0 60px 10px rgba(0,0,0,0.15)',
                            ],
                            scale: [1, 1.15, 1],
                        }}
                        transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' }}
                        className="relative w-3 h-3 rounded-full bg-black"
                    />
                </motion.div>
            </div>
